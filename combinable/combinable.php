...
class qtype_combined_combinable_mcallornothing extends qtype_combined_combinable_accepts_vertical_or_horizontal_layout_param {
    ...
    public function validate() {
        ...
        if (count($nonemptyanswerblanks) < 2) {
            $errors[$this->form_field_name("answergroup[0]")] = get_string('err_youneedmorechoices', 'qtype_mcallornothing');
        }
        if (count(array_filter($this->formdata->correctanswer)) === 0) {
            $errors[$this->form_field_name("answergroup[0]")] = get_string('err_nonecorrect', 'qtype_mcallornothing');
        }
        return $errors;
    }
